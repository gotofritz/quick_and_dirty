 ...
 * folder to a destination folder. Say you have a repo with this structure:
 * Cartoons/
 *   Biene Maia/
 *     BM01.mp4
 *     BM02.mp4
 ...
 *
 *   captain_mandolin -c /pth/to/config
...
function getListOfFilesToCopy(instructions, config = {}) {
  ...
  const filesToCopy = instructions.reduce(
    (accumulator, instruction, refToInstruction) => {
      ...
      // with breadth 1 it will be a normal breadth-first search, but with 2:
      // america/argentina
      // america/bolivia
      // asia/afghanistan
      // asia/armenia
      ...
      if (!shouldSkipMainLoop) {
        ...
        // be more convenient to enter what we want as next file (especially if
